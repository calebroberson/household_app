...
- Household: Caleb (`3bf30ca6-ba9b-4a96-b8a7-b8232af3cdc5`) + Matthew (joins later, resolved dynamically in scripts as "the other household_members row"). `seed_household_chores.sql` (repo root) seeds all 99 real household chores across 8 areas (Kitchen, Half Bath, Living Room, Stairs, Entryway, Caleb's Room [private], Caleb's Bathroom [private], Home) — written and verified against the household's actual chore list, but run manually by the user in the Supabase SQL editor (not yet confirmed run as of 2026-09-14). Not idempotent — re-running it duplicates every chore row (areas are protected by a unique-name-per-household index, chores aren't). A second `do $$ ... $$` block at the bottom activates real Matthew/Caleb rotation alternation once Matthew joins (rotate chores seed Caleb-only until then).
